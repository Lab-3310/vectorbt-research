...
class BbandCrossMeanReversionStrategy(BaseStrategy):
    def __init__(self, strategy_class: str, strategy_config: str):
        ...
        self.filter_ma_p = self._config.get(base_strategy_enum.PREPARE_DATA_PARAM, {}).get("filter_ma_p", None)
    ...
    def run_backtest(self):
        ...
        self.backtest_df['filter_ma'] = ta.SMA(self.backtest_df['low'], timeperiod=self.filter_ma_p1)
        ...
        bull_trend = (self.backtest_df[CLOSE] > self.backtest_df['filter_ma'])
        bear_trend = (self.backtest_df[CLOSE] < self.backtest_df['filter_ma'])
        ...
        confirm_long_signal = bband_upper_crossup & bull_trend
        confirm_short_signal = bband_upper_crossup & bear_trend

        # 3. Define the Long/Short in Entry/Exit 
        self.backtest_df['entry_long'] = np.where(confirm_long_signal, True, False)
        self.backtest_df['entry_short'] = np.where(confirm_short_signal, True, False)
